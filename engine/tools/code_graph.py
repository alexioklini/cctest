# Extracted from claude_cli.py — code structure graph (tree-sitter AST)
#
# Cross-module dependencies (must be supplied by the host module / import shim):
#   AGENTS_DIR      — str, path to the agents directory (from claude_cli.py)
#   _ok(result)     — formats a success response string
#   _err(msg)       — formats an error response string
#   _run_delegate() — LLM delegate helper used by generate_summaries()
#   _models_config  — dict of model configs used by generate_summaries()
...
# ---------------------------------------------------------------------------
# Placeholders — replace with real imports when integrating into claude_cli.py
# ---------------------------------------------------------------------------

def _ok(result):  # noqa: F811 — override in host
    import json
    return json.dumps({"ok": True, "result": result})


def _err(msg):  # noqa: F811 — override in host
    import json
    return json.dumps({"ok": False, "error": msg})


# _run_delegate and _models_config must be injected by the host module.
_models_config: dict = {}


def _run_delegate(*args, **kwargs):
    raise NotImplementedError("_run_delegate must be provided by the host module")
