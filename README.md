...
|---|---|---|
| 8.23.1 | 2026-05-02 | **Models tab UX + thinking format fix**. (1) Provider groups in the Models tab now start **collapsed** by default — click the ▶ header to expand. (2) Models within each provider are sorted **enabled-first alphabetically**, then disabled alphabetically — no more hunting through disabled entries. (3) `_detect_thinking_format` extended to classify `mistral-medium-3*` / `mistral-medium-latest` / `mistral-medium-2604` as `mistral_blocks` — previously these weren't in the pattern list so auto-detect seeded new entries with `none`, meaning the thinking button showed "model does not support thinking" even after manually configuring `mistral_blocks` (the live server's in-memory config hadn't been updated). |
