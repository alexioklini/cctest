...
### Intelligence
- **MemPalace memory** — single `mempalace_query` tool with hybrid BM25+vector+closet ranking. `mempalace-miner` daemon auto-mines configured source dirs every 30 min. Chat turns are mirrored to memory on demand (the `save_chat_to_memory` tool / per-turn memorize — the old `mempalace-chat-sync` daemon is retired). No manual mining, no MCP subprocess
