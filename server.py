...
class BrainAgentHandler(BaseHTTPRequestHandler):
    ...
    def _handle_chat(self):
        ...
        if _use_sdk:
            ...
            try:
                # Proxy SSE from sidecar to client
                import select as _sel, subprocess as _subp
                # Run proxy in subprocess to avoid handler thread interference
                _proxy_script = f'''
import socket, json, select, sys
sock = socket.create_connection(("127.0.0.1", 8421), timeout=300)
sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
payload = sys.stdin.buffer.read()
http_req = f"POST /query HTTP/1.1\\r\\nHost: 127.0.0.1:8421\\r\\nContent-Type: application/json\\r\\nContent-Length: {{len(payload)}}\\r\\n\\r\\n".encode() + payload
...
while True:
    ...
    if b"_result" in data: break
sock.close()
'''
                _proc = _subp.Popen(
                    [sys.executable, "-c", _proxy_script],
                    stdin=_subp.PIPE, stdout=_subp.PIPE, stderr=_subp.DEVNULL)
                ...
                while True:
                    _ready, _, _ = _sel.select([_proc.stdout], [], [], 5.0)
                    if not _ready:
                        if _proc.poll() is not None: break
                        continue
                    _chunk = _proc.stdout.read1(4096) if hasattr(_proc.stdout, 'read1') else _proc.stdout.read(4096)
                    if not _chunk:
                        break
                    _sse_buf += _chunk

                    while b"\n\n" in _sse_buf:
                        _block, _sse_buf = _sse_buf.split(b"\n\n", 1)
                        _block_str = _block.decode("utf-8", errors="replace").strip()
                        _evt_type = ""
                        _evt_data = None
                        for _line in _block_str.split("\n"):
                            if _line.startswith("event: "):
                                _evt_type = _line[7:].strip()
                            elif _line.startswith("data: "):
                                try: _evt_data = json.loads(_line[6:])
                                except json.JSONDecodeError: pass

                        if _evt_type in ("text_delta", "thinking_delta", "tool_call", "tool_result"):
                            _sse_out = _block + b"\n\n"
                            try:
                                self.connection.sendall(_sse_out)
                            except (BrokenPipeError, ConnectionResetError, OSError):
                                _proc.kill()
                                break
                            if event_callback and _evt_data:
                                event_callback(_evt_type, _evt_data)

                        if _evt_type == "_result" and _evt_data:
                            r = {
                                "text": _evt_data.get("text", "") or _full_text,
                                "sdk_session_id": _evt_data.get("sdk_session_id"),
                                "tokens_in": _evt_data.get("tokens_in", 0),
                                "tokens_out": _evt_data.get("tokens_out", 0),
                                "cost": _evt_data.get("cost", 0),
                                "tools": _evt_data.get("tools", []) or _tool_calls,
                            }
                            break
                        elif _evt_type == "text_delta" and _evt_data:
                            _full_text += _evt_data.get("text", "")
                        elif _evt_type == "tool_call" and _evt_data:
                            _tool_calls.append(_evt_data)

                _proc.wait(timeout=5)
                if not r.get("text") and _full_text:
                    r["text"] = _full_text
                if not r.get("tools") and _tool_calls:
                    r["tools"] = _tool_calls

                # Remove the old inlined proxy code
                if False:
                    _sidecar_sock = socket.create_connection(
                    (sdk_backend.SIDECAR_URL, sdk_backend.SIDECAR_PORT), timeout=300)
                _sidecar_sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                _http_req = (
                    f"POST /query HTTP/1.1\r\n"
                    f"Host: {sdk_backend.SIDECAR_URL}:{sdk_backend.SIDECAR_PORT}\r\n"
                    f"Content-Type: application/json\r\n"
                    f"Content-Length: {len(payload)}\r\n"
                    f"\r\n"
                ).encode() + payload
                _sidecar_sock.sendall(_http_req)

                # Read sidecar response headers
                _hdr = b""
                while b"\r\n\r\n" not in _hdr:
                    _b = _sidecar_sock.recv(1)
                    if not _b: break
                    _hdr += _b

                # Stream SSE from sidecar to client using select()
                _sidecar_sock.setblocking(False)
                _sse_buf = b""
                _full_text = ""
                _tool_calls = []
                _partial_tools_from_stream = []
                r = {"text": "", "sdk_session_id": None, "tokens_in": 0,
                     "tokens_out": 0, "cost": 0.0, "tools": []}

                while True:
                    _ready, _, _ = _sel.select([_sidecar_sock], [], [], 5.0)
                    if not _ready:
                        continue
                    try:
                        _chunk = _sidecar_sock.recv(4096)
