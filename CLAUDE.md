...
All chat + non-interactive LLM calls run **in-process** via `engine/llm_loop.py` (the OpenAI `/v1/chat/completions` wire path — where upstream prompt caching reliably hits). The Anthropic-SDK **sidecar subprocess + `server_lib/tool_mcp.py` + `.venv_sdk` were deleted in v9.247.0** (see `OPENAI_INPROCESS_LOOP_HANDOVER.md`); **CLIProxyAPI was removed in v9.278.0**. **Since 2026-07-19 ALL model traffic routes through the standalone `llm-router` service** (own repo `~/Documents/dev/llm-router`, port 8424, launchd `com.llm-router.server`, GUI `llmrouter.alexklinsky.dev`): provider keys, wire selection (OpenAI vs Anthropic upstream), coding-plan/credit tracking and per-request metrics live in the ROUTER, not here. Brain has exactly TWO provider entries — `llm-router` (cloud) and `llm-router-local` (`is_local:true` for gemma/whisper; **`is_model_local()` is provider-based**, this split keeps the GDPR local-fallback + quota-bypass + LocalProviderQueue semantics). Image generation (`engine/tools/image_gen.py`) also routes through the router since 9.380.0: `_image_api()` resolves the DEFAULT provider and calls `{base}/agents|/conversations|/files/<id>/content` (router special-passthrough paths; the agent's `model` stays the upstream Mistral id, forwarded verbatim). Brain model ids stayed unchanged; `base_model_id` = the router's model id (router maps to the real upstream id). Legacy disabled models with dead provider refs were deleted + tombstoned on 2026-07-19. Brain owns tool registry wiring + dispatch, MemPalace/scheduler/projects/MCP routing, runtime classes, AND the agentic loop. Providers are plain OpenAI-compatible `config.json` entries; the loop hits `{base_url}/chat/completions` regardless of the provider's `type` field. NB: upstream prompt caches (Kilo + Mistral) only engage on real conversation shapes — synthetic identical-prompt curl repeats report `cached_tokens: 0`; verify caching with multi-turn sessions.
...
`resolve_provider_for_model(model)` is the **single source of truth** for `{api_key, base_url, provider_name}` (chat, delegate, scheduler, warmup, background). Providers are plain OpenAI-compatible `config.json → providers` entries — since 2026-07-19 effectively only the two `llm-router*` entries (see Architecture); per-upstream keys/wires/plans are managed in the router's own GUI, and STT/TTS/OCR/voices also route through it (`/v1/audio/*`, `/v1/ocr`). Upstream-provider changes (keys, new providers, model sync) happen in the ROUTER, then brain syncs its catalog from `GET :8424/v1/models`.

**Provider-scoped IDs**: when multiple providers serve one model, stored as `provider/model_id` with `base_model_id`. Historical scoped ids (`OMLX/*`, `Bifrost/*`, `mistral/*`) still route. Bifrost retired 8.5.0. NB: `models/config action=update` REPLACES the whole model entry (no merge) — always send the full config.
...
- Memory is MemPalace **direct, not MCP**: `mempalace_query` (+ `save_chat_to_memory` + the `mempalace_kg_*` tools). The `mempalace_get_drawer`/`mempalace_list_drawers` pair was rolled back — they exist NOWHERE in `TOOL_DEFINITIONS`/`TOOL_GROUPS`/`TOOL_DISPATCH`; only stale names survive in exclude/deanon sets. Don't reintroduce them (mempalace-review L5).
...
- The structured key/value **memory tools** (`memory_store`/`memory_recall`/`memory_delete`/`memory_shared`) were **RETIRED by the Wiki migration** (Phase 2, 8cd8b249) — the editable **LLM Wiki** (`wiki_read`/`wiki_write`/`wiki_structure`/`wiki_delete`) is the agent's named-item memory now, and the `memory` group holds only the MemPalace tools (`mempalace_query`, `save_chat_to_memory`, `mempalace_kg_*`). Don't reintroduce `memory_*` tool names; docs/tests referencing them are stale (a characterization test carried `memory_recall` in the dedup-exempt assert until 9.314.2).
...
Imported as Python package — no MCP, no subprocess. **Vocabulary**: Drawer (atomic verbatim chunk ~800 chars) / Closet (index layer boosting search) / Room (topic bucket) / Wing (namespace). **Wing scheme** (ID-only): `user__<uid>` (private), `team__<tid>` (shared), `project__<pid>` (strictly isolated), bare names (shared). `mempalace_query` force-scopes to `project__<id>` when in a project (refuses if id missing rather than leak). **Storage**: pluggable backend selected via `MEMPALACE_BACKEND` env (set in the launchd plist). **Now on Qdrant** (own native process on `localhost:6333`, WAL-backed transactional ANN — `MEMPALACE_BACKEND=qdrant` + `MEMPALACE_QDRANT_URL`), with **scalar int8 quantization** (`MEMPALACE_QDRANT_QUANTIZATION=int8`: 4× RAM cut, int8 index in RAM + float32 originals on-disk, rescore+2× oversampling keeps recall ~98-99%; verified self-match 1.0 + eval parity vs unquantized). Vectors are embedded **Brain-side via MLX** (`embeddinggemma-300m`, `MEMPALACE_EMBEDDING_DEVICE=mlx` — NEVER auto/coreml here, that's 100% NaN); Qdrant needs no GPU. The quantization knobs are a venv patch on `backends/qdrant.py` (`# BRAIN-PATCH`, config/env-gated, default OFF — re-apply on every mempalace upgrade, see [[project_mempalace_venv_patches]] + `QDRANT_MIGRATION_PLAN.md`). A second venv patch (`scripts/patch_mempalace_miner.py`) makes the miner ROLL BACK partial sub-batch writes on failure, so a failed re-mine can never leave a file half-indexed with the current `source_mtime` (mempalace-review M6 — the mtime pre-filter would otherwise skip it forever). ONE shared collection per type (`mempalace_drawers` / `mempalace_closets`), wing is a metadata filter — the per-wing-collection experiment (v9.62.0) was REVERTED in v9.70.0. **History**: embedded ChromaDB (and the brute-force `sqlite_exact` interim) were retired here because the embedded-Chroma HNSW segment files raced the 3 writer daemons → corruption (root cause: a vendored `backends/chroma.py` quarantine-validator bug; the `sqlite_exact` no-index backend dodged it but gave up ANN); Qdrant's transactional WAL makes the concurrent-writer-corrupts-half-flushed-file mode structurally impossible. Two writer daemons (all in `server_daemons.py`, each single-threaded): `mempalace-miner` (artifact ingestion) + `mempalace-project-sync` (input folders + web-URLs). The third, `mempalace-chat-sync` (mirrored chat turns/summaries/attachments/allowlisted tool_results to wings), was **RETIRED** (v9.414.x, LLM-Wiki migration — its loop returns at `server_daemons.py:645` and server.py no longer launches it); chat turns reach the palace ON DEMAND instead: the `save_chat_to_memory` tool's immediate sync (`server.py:_save_chat_to_memory_callback`, cursor-clamped) and the per-turn `memorize_turns`/`purge_turns` actions + session-delete purge.
- **Cross-daemon write serialization — `_palace_write_lock`** (`server_daemons.py:63`, a blocking `RLock`): held around EVERY palace mutation — `mp_miner.mine()`, `tool_add_drawer()`, stale-purge `col.delete(ids=...)`, the live immediate chat-sync + per-turn memorize, the session/turn purge, the profile mirror purge+add, sched-artifact writes, AND the sqlite-rebuild swap (mempalace-review M5) — across the daemons AND the on-demand writers. The package's own `mine_palace_lock` is per-palace but **non-blocking** (concurrent writer gets `MineAlreadyRunning` = a silently-refused write), so Brain wraps writes in its own lock to make writers QUEUE instead of collide. Coarse by design (a multi-second mine blocking a chat-sync cycle is the correct trade); serialized-but-late beats raced-and-corrupt. Originally added because, on embedded ChromaDB, a bulk delete racing an upsert wedged the shared HNSW segment ([[project_chroma_bulk_delete_corruption]]); on Qdrant (WAL-backed, no in-process segment files) that corruption mode is gone, but the lock is retained to keep daemon writes serialized + cheap.
...
- **Chat-sync write-loss guard** (mempalace-review H2): the LIVE immediate-sync cursor (`server.py:_save_chat_to_memory_callback._do_sync`) is clamped BELOW the lowest message id whose drawer write genuinely failed (lock-refused/exception, distinct from dedup), so a transient failure retries on the next sync instead of being skipped forever ([[project_mempalace_review_findings]]). The retired chat-sync daemon's dead-body guard (`server_daemons.py:1002`) is the same design.
