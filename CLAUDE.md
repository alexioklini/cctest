...
All chat + non-interactive LLM calls run **in-process** via `engine/llm_loop.py` (the OpenAI `/v1/chat/completions` wire path — where upstream prompt caching reliably hits). The Anthropic-SDK **sidecar subprocess + `server_lib/tool_mcp.py` + `.venv_sdk` were deleted in v9.247.0** (see `OPENAI_INPROCESS_LOOP_HANDOVER.md`); **CLIProxyAPI was removed in v9.278.0**. **Since 2026-07-19 ALL model traffic routes through the standalone `llm-router` service** (own repo `~/Documents/dev/llm-router`, port 8424, launchd `com.llm-router.server`, GUI `llmrouter.alexklinsky.dev`): provider keys, wire selection (OpenAI vs Anthropic upstream), coding-plan/credit tracking and per-request metrics live in the ROUTER, not here. Brain has exactly TWO router provider entries — `llm-router` (cloud) and `llm-router-local` (`is_local:true` for gemma/whisper; **`is_model_local()` is provider-based**, this split keeps the GDPR local-fallback + quota-bypass + LocalProviderQueue semantics) — plus `mistral-direct` as a **credential-holder ONLY** for `engine/tools/image_gen.py` (scans providers for `mistral.ai` in base_url; its base_url deliberately has NO `/v1` so model-sync discovers nothing there; zero models point at it). Brain model ids stayed unchanged; `base_model_id` = the router's model id (router maps to the real upstream id). Brain owns tool registry wiring + dispatch, MemPalace/scheduler/projects/MCP routing, runtime classes, AND the agentic loop. Providers are plain OpenAI-compatible `config.json` entries; the loop hits `{base_url}/chat/completions` regardless of the provider's `type` field. NB: upstream prompt caches (Kilo + Mistral) only engage on real conversation shapes — synthetic identical-prompt curl repeats report `cached_tokens: 0`; verify caching with multi-turn sessions.
