...
**Two mirrored implementations** (regex-only, must stay in sync): `PIIScanner` in `web/index.html`; `_pii_rules()` + `_pii_scan_text()` + `_pii_scan_bare_identifiers()` in `brain.py`.
