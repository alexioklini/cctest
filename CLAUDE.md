...
Imported as Python package — no MCP, no subprocess. **Vocabulary**: Drawer (atomic verbatim chunk ~800 chars) / Closet (index layer boosting search) / Room (topic bucket) / Wing (namespace). **Wing scheme** (ID-only): `user__<uid>` (private), `team__<tid>` (shared), `project__<pid>` (strictly isolated), bare names (shared). `mempalace_query` force-scopes to `project__<id>` when in a project (refuses if id missing rather than leak). **Storage**: ONE shared ChromaDB collection (`mempalace_drawers` / `mempalace_closets`), wing is a metadata filter — the per-wing-collection experiment (v9.62.0) was REVERTED in v9.70.0 (didn't fix the HNSW corruption; root cause was a vendored `backends/chroma.py` quarantine-validator bug, see [[project_mempalace_venv_patches]]). Three writer daemons (all in `server_daemons.py`, each single-threaded): `mempalace-miner` (artifact ingestion) + `mempalace-chat-sync` (mirrors chat turns/summaries/attachments/allowlisted tool_results to wings) + `mempalace-project-sync` (input folders + web-URLs).
- **Cross-daemon write serialization — `_palace_write_lock`** (`server_daemons.py:62`, a blocking `RLock`): held around EVERY palace mutation — `mp_miner.mine()`, `tool_add_drawer()`, stale-purge `col.delete(ids=...)` — across all three daemons. The package's own `mine_palace_lock` is per-palace but **non-blocking** (concurrent writer gets `MineAlreadyRunning` = a silently-refused write), so Brain wraps writes in its own lock to make daemons QUEUE instead of collide. Coarse by design (a multi-second mine blocking a 60s chat-sync cycle is the correct trade); serialized-but-late beats raced-and-corrupt. Without it, a bulk delete racing an upsert wedges the shared HNSW segment ([[project_chroma_bulk_delete_corruption]]).
