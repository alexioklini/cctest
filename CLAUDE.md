...
- **Server prefetch** (`_handle_chat`): fetches each URL via `tool_web_fetch`, prepends a markdown preamble to the user message (no `read_document` tool round — local models skip mandated fetches, see [[project_exa_search_only_gemma_fetch_skip]]). The preamble is part of `message`, so `_ALLOWED_MSG_KEYS` strips it from the wire/DB like any user text.
