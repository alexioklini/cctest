...
- **CITATION DISCIPLINE**: every factual claim from the project must be paired with `[Quelle: <basename> — "<wörtliches Zitat 10-25 Wörter>"]` right after the claim. The verbatim quote is mandatory — the user can Cmd+F the original PDF and verify. Without a quote, the citation is unverifiable and not allowed. Locator additions (`Page N` for PDFs, `Slide N` for PPTX, `Sheet "Name"` for XLSX) are allowed only when genuinely visible in the read_document text. The `.md` companions don't preserve original paragraph numbering, so any `§N` is fabricated; explicit anti-pattern in the prompt forbids them. `.brain-extracted/<name>.<ext>.md` source paths must be cited as the original binary's name.

**Anti-room-name-guessing**: the `mempalace_query` tool's `room` parameter description was a permissive list of speculative example rooms ('document', 'documentation', etc.) which the model used as valid vocabulary, returning zero drawers and producing false 'not in the documents' answers. Description rewritten to enumerate the real rooms Brain's miner uses (`general` for policy/document content, `artifacts`, `chat`/`chat_summary`/`chat_attachment`, `reference`) and explicitly forbid invention.

**Validated sampling defaults for Mistral Small** (`config.json`, gitignored): `temperature: 0.2`, `top_p: 0.85`. Captured from six measured runs on the same canary; the combination dropped fabricated formulae, paragraph numbers, intervals, and thresholds while preserving correct retrieval and citations. `temperature: 0` was tested (Mistral provider rejected the request); `temperature: 0.1` showed no measurable improvement over 0.2. The chat composer + Models tab UIs honor per-model temperature/top_p so other models can be tuned independently.
...
- **Schema** (`project.json`): `input_folders: [{path, recursive, auto_sync, added_at}]` (auto_sync default true; v8.21.4), `input_folders_last_scan`, `sync_status: {state, last_run_started, last_run_finished, last_files_filed, total_indexed, total_files (v8.21.4 — distinct source_file count), total_triples, last_folders_seen, last_error, items: {<key>: {kind, id, state, drawers_filed, error, ...}}}`. `items` keyed `attachment:<src_hash>` or `folder:<abs_path>`. Live snapshot `_project_sync_live[(agent, project)]` carries `cycle_total_files` + `cycle_processed_files` + `started_at` mid-cycle for the UI's progress bar / ETA.
- **Endpoints** (manage-gated except `sync-status` which is read-gated): `GET/POST /v1/agents/<id>/projects/<name>/input-folders`, `POST .../input-folders/<idx>` (v8.21.4 — partial update for path/recursive/auto_sync), `DELETE .../input-folders/<idx>`, `POST .../sync-now`, `GET .../sync-status`. `_handle_project_sync_status` layers `_project_sync_live` (in-flight state) on top of the persisted `sync_status` so the UI sees state changes mid-cycle, and additionally returns `interval_seconds` + derived `next_run_at` (= `last_run_finished + interval_seconds`) so the chip can show a countdown without a second config fetch.
...
- **Daemon** (`mempalace-project-sync` thread, server.py): polls every `mempalace.project_sync.interval_seconds` (default 21600). On every tick: enumerates all `(agent, project)` pairs; processes any in `_project_sync_requests` first (drained via `_project_sync_wakeup`); per project resolves `wing = project__<id>`, ensures `mempalace.yaml` matches the expected wing (rewrites on drift), mines `ingested/` then each input folder via `mp_miner.mine(wing_override=wing)`. Per-folder cap (`max_files_per_folder`, default 5000) refuses runaway home-dir picks at the top level. **Authoritative drawer counts** via `_count_wing_drawers_by_source(wing, source_prefix)` — Chroma metadata `where={"wing": wing}` plus in-Python `source_file.startswith` — and `_count_wing_drawers_total(wing)` / `_count_wing_files_total(wing)` (v8.21.4 — distinct source_file count, used by the chip's `N files` label). Persists `total_indexed` (cumulative, survives dedup-only re-runs) plus per-item `drawers_filed`. Without this the miner's "Drawers filed: 0" reply on every cycle after the first would clobber the persisted count back to zero. **Auto-sync gate** (v8.21.4): when an entry has `auto_sync=false`, the daemon skips it on scheduled cycles — sets the row state to `paused`, contributes 0 to `cycle_total_files`. The gate is **bypassed** when the project is in the `requested` set (manual "Sync now"), so the user can still run on demand. **Cycle progress instrumentation** (v8.21.4): `cycle_total_files` is computed via a cheap pre-walk (recursive `os.walk` for `recursive=true`, top-level `os.scandir` otherwise) before mining begins; `_bump_processed(n)` advances `cycle_processed_files` after each attachment batch and after each folder finishes, including error-skip paths so the progress bar never stalls below total. Slight overshoot is by design — the miner's gitignore + extension filter narrows the actual count, but undershooting would freeze the bar at 100%.
...
- **UI** (`web/index.html`): project header gets a `Memory: …` chip — pulses blue when syncing, green when idle, red on error; click triggers `projectSyncNow()`. **Idle label** (v8.21.4): `Memory: N files · M relations · next sync in Xh` (was `N indexed · M triples`). `total_files` is the unit users count, `total_indexed` (drawer count) moves to the tooltip. **Syncing label**: `Memory: syncing P/T files · ETA Xm (current folder)` — `humanIn(next_run_at)` for countdown, ETA elapsed-extrapolated from `started_at` and gated to ≥5% progress so an early 0/N doesn't claim 12 days. **Triples renamed to "relations"** in every user-facing surface (chip, per-folder pill, KG sub-badges); admin Settings → Knowledge Graph keeps the technical name. Right panel grows an "Input folders" section (next to Files) with `+ Add` (opens a stacked filesystem-browser modal `_pifLoadFolder` reusing `/v1/files/tree`, with recursive + auto-sync checkboxes). **Row layout** (v8.21.4): three-line block — name (last path segment, bold) + edit pencil + delete × on top, full absolute path in mono with RTL ellipsis (keeps the tail visible) below, badges wrap-flex on the third line (`recursive` / `top-level`, `auto-sync off`, sync state pill, KG sub-badge). Edit pencil opens a modal with the same picker shell as Add, prefilled with the current path/recursive/auto_sync; saves via `POST /input-folders/{idx}`. Delete shows an amber-warning modal with red `Remove folder` button (replaces bare `confirm()`). Files section gets a status pill per attachment. Pills repaint every 5s from `/sync-status` polling via `paintProjectItemPills()` walking `data-pif-pill` nodes — preserves DOM identity (no flicker, no list re-render). Pill colours: indexed (green), syncing (blue, pulsing), pending (grey), error (red), paused (amber, v8.21.4). **Right pane is draggable** (v8.21.4): `.project-detail-panel-resize-handle` on the left edge mirrors `#right-panel`'s pattern; width persisted to `localStorage('project-detail-panel-width')`, clamped 240–640px. Init runs from `openProject()`; `_bound` flag prevents double-binding. **Project composer placeholder** (v8.21.4): `loadProjectDetail` overrides the textarea's static `placeholder` with `Write your message to <project.name>` (falls back to slug). **Instructions empty-state** updated from "customize Claude's responses" to "customize Brain Agent's responses" in all three rendering paths.
