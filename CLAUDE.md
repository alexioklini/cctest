...
Sidebar peer of Translation/Projects — an in-house data workbench. A workbench session **is a real chat session** created via `SessionManager`, just flagged `session.is_data_workbench = True` (re-derived on load from `DataSessionDB.get(sid) is not None`). The conversation goes through the ordinary `/v1/chat` endpoint; `/v1/data/*` only handles session-create, file upload, table listing.

- **DuckDB-in-artifact-folder invariant**: each workbench session's tables live in `agents/<agent>/artifacts/<date>_<sid_prefix>/_data.duckdb` — the same folder `python_exec` runs in (CLAUDE.md cwd invariant). Tool/handler code resolves it via `_get_artifact_session_folder(sid)` + `AGENTS_DIR/<agent>/artifacts/<folder>`, NOT process cwd (the daemon never chdir's). Cleaned up by the empty-session purge.
- **`data_workbench` thread-local gating**: `handlers/chat.py`'s worker sets `engine._thread_local.data_workbench` from `session.is_data_workbench`; `_build_system_prompt` injects the `DATA_WORKBENCH_PROMPT` block only when it's set, and `_get_agent_tool_names` adds the `data_viz` group only then — so the warm-pool KV prefix stays workbench-agnostic for ordinary chats (mirrors the `project` gating). `DATA_WORKBENCH_PROMPT` lives in `engine/tools/data_viz_prompts.py`; brain.py imports it.
- **`data_viz` tool group** (`data_query`, `data_anonymise`, `data_scan_files`, `data_render_chart`): bodies in `engine/tools/data_viz.py`, imported into brain.py near `tool_generate_image` (the engine module is the only live tool-extraction besides `image_gen`). `data_query` runs **read-only SQL** against `_data.duckdb` (SELECT/WITH/DESCRIBE/SHOW/PRAGMA/SUMMARIZE/EXPLAIN — refuses anything else; `register_as` is the one permitted CREATE). New tool → 4 edit sites in brain.py: `TOOL_DEFINITIONS`, `TOOL_GROUPS`, the import + `TOOL_DISPATCH`. `data_viz` is NOT in `DEFAULT_TOOL_GROUPS` or `main`'s `token_config.tool_groups` — it's workbench-only.
- **`data_render_chart`** (`{spec, table, scale?}` → `{ok, table, spec, png_b64, n_rows}`): binds a Vega-Lite v5 spec to one of the workbench's DuckDB tables — inlines a ≤5000-row sample as `data.values` (the model must NOT put a `data` key in the spec it passes), checks every encoded `field` exists in the table (skipped if the spec uses `calculate`/`aggregate`/`fold`/`window`/`joinaggregate` transforms — Vega validates those on render), renders a PNG server-side via **vl-convert** (`vl_convert.vegalite_to_png` — a self-contained Rust wheel, no node/headless browser), saves it as a `chart_N.png` artifact, emits a `data_chart` SSE event, audits `data_render_chart`. Raises on an invalid spec / a missing column so the agentic loop hands the error back → the model retries. Endpoint `POST /v1/data/sessions/<sid>/render` (no-LLM, re-render an edited spec — same body). Web UI: a "Chart builder (Vega-Lite)" pane in the Data view — table picker, bar/line/scatter templates, editable spec textarea, "Preview (client)" via vega-embed (CDN, eager-loaded) over the ≤20-row sample, "Render PNG (server)" via `/render`. Chart-semantics cheat-sheet (Brain-original, NOT a verbatim Data-Formulator copy) lives in `engine/tools/data_viz_prompts.py`. Canary: `eval/data/` (10 prompts, deterministic scorer — checks the `data_render_chart` call's mark + encoded fields; the 2 `A*` questions pass when the model clarifies instead of charting). Deps: `pip install --break-system-packages vl-convert-python` (jsonschema already present — unused, vl-convert validates on render).
- **`data_anonymise`** (one body, two modes — deterministic Python, NO LLM in the loop; the caller supplies the column→strategy plan): `mode="anonymise"` — per-column strategies `hash`/`tokenise`/`redact`(regex-replace PII inside free text via `_pii_rules()`)/`generalise`(opts.kind=date_month|date_quarter|age_band|postcode_district|bucket+size)/`nullify`/`shuffle`/`noise`. **Invariants enforced in code**: never mutates source — always a new DuckDB table + a new output file; writes a 3-sheet index workbook (`mapping`/`schema`/`info`) inside the artifact folder when a reversible strategy (`tokenise`, or `hash` with `embed_salt`) is used; re-runs the 71-detector scanner on the output → `residual_scan` (names exactly what still leaked); k-anon warning if generalisation leaves quasi-identifier groups < `k` (default 5); one audit line per run. `output_format`: `preserve` (xlsx→openpyxl roundtrip preserving sheets/formatting, only treated cells changed — needs `source_file`; the faithful chunked-OOXML rewrite for docx/pptx lands in a later PR; csv→DuckDB-style copy) / `csv` / `xlsx` / `markdown`. `mode="deanonymise"` — `source_file` + `index_file` in → reads the `schema` sheet for treated columns (matched by normalized header then position), `mapping` sheet for surrogate→original, substitutes back, writes a restored file; one-way strategies returned in `not_reversible[]`. Endpoints: `POST /v1/data/sessions/<sid>/anonymise` + `…/deanonymise` (thin no-LLM calls into the tool body — same Python, two front doors; deanonymise + the index download are owner/admin only), `GET …/mapping/<file>` (download the index workbook — **the download is itself an audit event `mapping_downloaded`**, it's the re-identification key). Web UI: "Anonymise…" / "Deanonymise…" modals in the Data view (column table + per-column strategy + opts JSON + output-format selector + the residual-scan / index-file result panel).
- **`DataSessionDB`** in `server_lib/db.py` (`data_sessions` table) — index of which tables each workbench has + title/owner for the History UI. `.init()` called from `SessionManager.__init__` alongside `ChatDB.init()`/`FavouritesDB.init()`.
- **Upload** (`POST /v1/data/sessions/<sid>/upload`): xlsx → openpyxl per-sheet → pandas DataFrame → DuckDB table; csv → `read_csv_auto`. The 71-detector GDPR scanner runs over a ≤20-row sample → flagged columns reported + masked (`«PII»`) in the sample shown to the model; the full column stays in DuckDB. Original upload kept alongside the DuckDB. Audit line `data_upload`.
- **Deps pip-installed into the system python (homebrew, externally-managed) — no `requirements.txt`. New deps documented here**: `duckdb` (PR1). `vl-convert-python` will be added with the chart PR. `pymupdf`/`python-docx`/`python-pptx`/`pandas`/`openpyxl`/`jsonschema` are already present. Install with `pip install --break-system-packages <pkg>`.
- **`data_scan_files`** (§17, file-level GDPR scan): runs the 71-detector scanner over the workbench's DuckDB tables (PR3 scope: tabular sources — docx/pptx/pdf file scanning lands with the PR4 format arms). Returns per-table `{name, type, status: clean|dirty|error, findings: [{where, column, category, count, examples, suggested_strategy}], worst_category, total_hits}` + `summary`. **Modifies nothing.** Pure code, no LLM. Audit `files_scanned`. `tables` arg scopes it; omit = scan all. The suggested-strategy map (`_SUGGEST_STRATEGY` in `engine/tools/data_viz.py`): contact/secrets/network→redact, financial→hash, personal/national_id/bare_id→tokenise. Endpoint `POST /v1/data/sessions/<sid>/scan`. Web UI: "Scan for GDPR…" → triage modal listing dirty tables with per-table Auto-fix (calls `…/anonymise` with the suggested strategies, re-scans, shows whether the rescan came back clean) / Fix-in-GUI (opens the Anonymise modal pre-loaded with that table) + bulk "Auto-fix all dirty". (Project-input-folder scanning + the path-traversal guard for arbitrary paths is deferred — PR3 scans workbench tables only.)
- Not built yet (later PRs): the faithful chunked-OOXML `preserve` rewrite for docx/pptx + PDF redaction + docx/pptx/pdf file-scanning (PR4, with the `rewrite_office_zip` skeleton refactor), `data_transform`/`data_verify`/`data_compare`/`data_report`, History/Reports UI, GDPR-triggered `auto_anonymise`.
