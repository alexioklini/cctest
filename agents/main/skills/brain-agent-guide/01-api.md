...
- `GET /v1/sessions/<id>/pii-decisions-view` — DB-ONLY modal data (9.204.6): one row per DECIDED value (latest decision = current status) from the `pii_decisions` ledger, NO live re-scan → `{items:[{rule_id,label,category,masked,value_hash,source,source_label,status,false_positive,turn_action,fake_value,history}], counts:{status:N}, item_count}`. `status` ∈ open|anon|accepted|local|fp. This is what `openPiiHistoryModal` renders — reading the ledger (not a live scan) avoids the phantom-"open" duplicates a re-scan caused when its string form for a value differed from the stored decision.
