...
CHANGELOG = [
    ("5.0.0", "2026-03-28", "Full SDK migration complete — closed all remaining gaps from the Agent SDK transition. Phase 1: HTTP MCP server exposes 24 custom tools (memory, gmail, documents, code graph, delegation, skills, scheduling, nodes, exa) to the sidecar via /v1/tools/call. Phase 1.2–1.3: chat summaries and transcript indexing now trigger after SDK responses. Phase 2: file change watcher polls agent dirs every 10s to trigger QMD reindex, entity extraction, and KG updates for SDK file writes. Phase 3: rate limiting pre-check, model fallback on SDK error, plan mode and workflow tool restrictions enforced via SDK allowed_tools. Phase 4: trace spans and audit logging around sidecar calls. Phase 5: all background tasks (auto-memory, autodream, summaries, relationship discovery) route through SDK; query_sync extended with tool_defs, cancel support, sdk_session_id resume, and return_metadata; _run_delegate routes both tools=True and tools=False through sidecar. Phase 6: SDK PreToolUse/PostToolUse hook callbacks call /v1/hooks/run for pre-blocking and post-processing. TUI interactive, CLI one-shot, and scheduled tasks all route through SDK with graceful fallback to direct API."),
