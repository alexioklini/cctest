...
# Separators that split a filename basename into candidate name tokens. A
# filename joins name parts with these instead of spaces
# ('STARK_Bonnie_M_Mrs.', 'code-of-conduct'), so the body-text sweeps — which
# treat `_`/`-` as word characters and deliberately leave path forms intact —
# never fire on them. This helper is the ONE place that reaches inside a
# filename, and it does so path-safely (see below).
_FILENAME_TOKEN_SPLIT_RE = re.compile(r"[ _\-.,]+")


def pseudonymize_filename(basename: str, *, mapping: Mapping) -> str:
    """Return a path-safe pseudonymised form of a single filename basename.

    The attachment filename is a PII surface like any other ('STARK_Bonnie_M_
    Mrs._107625_Scan.pdf'), but it is ALSO a path the model must be able to
    hand back to read_document. Two constraints the body-text sweeps can't
    satisfy at once:

      * `apply_known_values` / `apply_entity_variants` treat `_`/`-` as word
        characters and EXPLICITLY skip filename forms (a rewritten path in a
        tool RESULT would break the roundtrip) — so a name joined by
        underscores is never matched.
      * `deanonymize_text` restores a plain name-fake ('Cameron') WORD-BOUNDED,
        so 'Cameron_Scan.pdf' would not restore either — the `_` blocks it.

    So this replaces each PII-bearing token with the mapping's OPAQUE token
    (`<KIND_N_salt>`, non-alphabetic → substring-restorable), never the
    shape-fake. The extension is left verbatim so read_document dispatches on a
    real suffix, and the args-deanon seam (deanonymize_text, substring) turns
    the opaque token back into the real stem before dispatch — the roundtrip is
    structural, not heuristic.

    Detection reuses the mapping the worker has ALREADY populated from the
    confirmed dialog decisions (the original filename was emitted as scanned
    content, so its name spans are decided + minted before this runs): a token
    is pseudonymised iff, joined-form or bare, it is a confirmed name/org/mrz
    original in `mapping.forward`. Non-PII tokens ('Code', 'Conduct', '107625',
    'Scan') stay verbatim. Registers `original_token -> opaque` so the same
    filename renders stably and restores everywhere (path, reply, tool args).

    Returns the rewritten basename (unchanged when nothing matched)."""
    if not basename or (not mapping.entities and not mapping.forward):
        return basename
    stem, ext = os.path.splitext(basename)
    if not stem:
        return basename
    parts = _FILENAME_TOKEN_SPLIT_RE.split(stem)
    seps = _FILENAME_TOKEN_SPLIT_RE.findall(stem)

    # Build a lowercase real-token → opaque-token map from the CONFIRMED
    # entities. The entity layer is authoritative: it holds every real name
    # part (person surname + givens, org stem tokens) whether or not that bare
    # token was registered as a standalone forward pair. That matters — a short
    # given-name fake ('Sam', 3 chars) is deliberately NOT registered standalone
    # in the body tables (collision caution, 9.383.6), so relying on
    # `mapping.forward` alone would leave 'Bonnie' visible in a filename. Here a
    # filename token is unambiguously a name part, so every entity token is
    # eligible.
    #
    # Set of lowercase real name tokens (person surname + givens, org stem)
    # eligible for filename pseudonymisation, tagged with their category.
    _eligible: dict[str, str] = {}
    for ent in mapping.entities.values():
        if _is_person_ent(ent):
            for t in [ent.get("sur", "")] + list(ent.get("givens") or []):
                t = (t or "").strip().lower()
                if len(t) >= 2:
                    _eligible.setdefault(t, "name")
        else:
            for t in ent.get("stem") or []:
                t = (t or "").strip().lower()
                if len(t) >= 2:
                    _eligible.setdefault(t, "organisation")

    def _opaque_for_surface(surface: str, cat: str) -> str:
        """Opaque token for THIS surface form of a name token in a filename.

        Registers ONLY the reverse direction (token → surface) so the
        reply-deanon and the read_document args-deanon restore the exact casing
        the filename used. The forward table is NOT touched: `forward[surface]`
        may already hold the body-text shape-fake ('STARK' → 'DAVIS'), and
        overwriting it would make body text render the opaque token too. A
        private forward alias (`\\x00file:<surface>`) caches the mint so the
        same surface renders stably across re-renders — it can never appear in
        real text, so it never collides with a body sweep. Opaque (contains
        '<') → substring-restorable through the surrounding underscores, unlike
        the alphabetic shape-fake."""
        alias = f"\x00file:{surface}"
        cached = mapping.forward.get(alias)
        if cached:
            return cached
        token = mapping.next_token(cat)
        mapping.forward[alias] = token
        mapping.reverse[token] = surface
        mapping.categories[alias] = cat
        mapping.derived.add(alias)
        return token

    out_parts = []
    changed = False
    for tok in parts:
        cat = _eligible.get(tok.lower()) if tok else None
        if cat:
            out_parts.append(_opaque_for_surface(tok, cat))
            changed = True
        else:
            out_parts.append(tok)
    # Reassemble stem with original separators preserved.
    rebuilt = out_parts[0]
    for i, sep in enumerate(seps):
        rebuilt += sep + out_parts[i + 1]

    # Non-name confirmed values that appear as a whole filename token but aren't
    # tokenised by the entity pass (a password/customer-number FILENAME — the
    # `Alcuatmisi02026!.txt` specimen — or any other opaque-token finding whose
    # value the scanner minted). Substring-replace over the reassembled stem,
    # OPAQUE fakes only: a shape-fake here would be word-bounded on the reverse
    # and break the path roundtrip. Skips the alias keys (private) and name/org
    # categories (already handled per-token, and their body fakes are
    # alphabetic). Longest-first so a value containing another replaces cleanly.
    _subst = sorted(
        (k for k, v in mapping.forward.items()
         if not k.startswith("\x00file:")
         and v and not v.isalpha()
         and mapping.categories.get(k) not in ("name", "mrz", "organisation")
         and len(k) >= 4),
        key=len, reverse=True)
    for k in _subst:
        if k in rebuilt:
            rebuilt = rebuilt.replace(k, mapping.forward[k])
            changed = True
