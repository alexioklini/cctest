...
class ChatDB:
    ...
    def init():
        ...
        with _db_conn() as conn:
            ...
            conn.execute(
                ...
                "error='Server restart — generation lost', "
                "finished_at=strftime('%s','now') WHERE status='generating'")
            # ── AI-generated SKILLS (draft, review-before-save) ──
            # Same draft/review-before-save contract as workflow_gen: the UI
            # polls status and on ready loads the fields into the skill editor —
            # nothing is written to user_skills/ automatically (POST /v1/skills/
            # save persists the reviewed draft as SKILL.md + skill.meta.json).
            conn.execute("""
                CREATE TABLE IF NOT EXISTS skill_gen (
                    id             TEXT PRIMARY KEY,
                    agent_id       TEXT NOT NULL DEFAULT 'main',
                    source_kind    TEXT NOT NULL DEFAULT 'nl',
                    source_ref     TEXT NOT NULL DEFAULT '',
            ...
            """)
            conn.execute(
                "UPDATE skill_gen SET status='error', "
                "error='Server restart — generation lost', "
                "finished_at=strftime('%s','now') WHERE status='generating'")
            # ── Deep Research runs ──
    ...
    @staticmethod
    @_db_safe(default=None)
    def create_skill_gen(gen_id, agent_id, source_kind, source_ref, created_by):
        """Insert a skill-generation run in 'generating' state; the worker flips
        it to ready/ready_with_warnings/error via update_skill_gen."""
        with _db_conn() as conn:
            conn.execute(
                "INSERT INTO skill_gen "
                "(id, agent_id, source_kind, source_ref, created_by) "
                "VALUES (?, ?, ?, ?, ?)",
                (gen_id, agent_id, source_kind, source_ref, created_by))
            conn.commit()
    ...
    @staticmethod
    @_db_safe(default=None)
    def update_skill_gen(gen_id, **fields):
        """Patch a skill-gen row. Whitelisted columns only; sets finished_at
        when status flips to a terminal state."""
        allowed = ("status", "phase", "slug", "display_name", "description",
                   "body_md", "notes", "warnings", "error", "model")
        sets, vals = [], []
        for k in allowed:
            if k in fields:
                sets.append(f"{k} = ?")
        ...
        with _db_conn() as conn:
            conn.execute(
                f"UPDATE skill_gen SET {', '.join(sets)} WHERE id = ?", vals)
            conn.commit()

    @staticmethod
    @_db_safe(default=None)
    def get_skill_gen(gen_id):
        with _db_conn() as conn:
            conn.row_factory = sqlite3.Row
            row = conn.execute(
                "SELECT * FROM skill_gen WHERE id = ?", (gen_id,)).fetchone()
            return dict(row) if row else None

    @staticmethod
    @_db_safe(default=None)
    def cancel_skill_gen(gen_id):
        """Set the cooperative-cancel flag — the worker checks it between phases."""
        with _db_conn() as conn:
            conn.execute(
                "UPDATE skill_gen SET cancel = 1 WHERE id = ?", (gen_id,))
            conn.commit()

    @staticmethod
    @_db_safe(default=False)
    def skill_gen_cancelled(gen_id):
        with _db_conn() as conn:
            row = conn.execute(
                "SELECT cancel FROM skill_gen WHERE id = ?", (gen_id,)).fetchone()
            return bool(row and row[0])
