...
// ---------- network ----------
// BRAIN-PATCH: this dashboard is served by brain-agent under /memdash/ — all
// /api/* calls are prefixed to /memdash/api/*, and auth rides Brain's own
// session token (localStorage['auth-token'] → Authorization: Bearer …) rather
// than the dashboard's X-Auth-Token. Re-apply on dashboard upgrade.
function brainApiUrl(url) {
  return (typeof url === "string" && url.startsWith("/api/")) ? "/memdash" + url : url;
}
function brainAuthToken() {
  try { return localStorage.getItem("auth-token") || ""; } catch { return ""; }
}
async function fetchJson(url, options = {}) {
  url = brainApiUrl(url);
  const headers = { ...(options.headers || {}) };
  const token = brainAuthToken();
  ...
  if (response.status === 401 || response.status === 403) {
    // BRAIN-PATCH: auth is Brain's, not the dashboard's — bounce to Brain's
    // main UI to (re)authenticate as an admin instead of the dashboard login.
...
async function loadPalace() {
  ...
  render();
  // BRAIN-PATCH: if a heavy wing is pre-selected (deep link), load its bodies.
  ensureWingLoaded(state.selectedWing).catch(() => {});
...
function renderNav() {
  ...
  els.wingNav.querySelectorAll(".nav-item").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      state.selectedDrawerId = null;
      render();
      // BRAIN-PATCH: heavy wings (e.g. brain_code) ship without bodies in the
      // default /api/palace payload to avoid freezing the browser. Load this
      // wing's drawers on demand, then re-render.
      await ensureWingLoaded(state.selectedWing);
    });
  });
}

// BRAIN-PATCH: fetch + merge a heavy wing's drawer bodies on demand.
async function ensureWingLoaded(wingName) {
  if (!wingName || wingName === "all" || !state.palace) return;
  const heavy = state.palace.heavy_wings || [];
  const loaded = state.palace.loaded_wings || [];
  if (!heavy.includes(wingName) || loaded.includes(wingName)) return;
  try {
    const data = await fetchJson(`/api/palace?wing=${encodeURIComponent(wingName)}`);
    const have = new Set(state.palace.drawers.map((d) => d.drawer_id));
    for (const d of (data.drawers || [])) {
      if (!have.has(d.drawer_id)) state.palace.drawers.push(d);
    }
    state.palace.loaded_wings = [...loaded, wingName];
    render();
  } catch (error) {
    console.error(`memdash: could not load wing ${wingName}:`, error.message);
  }
}
...
function renderDrawers() {
  ...
  // BRAIN-PATCH: hard cap the number of cards rendered into the DOM. Some wings
  // (brain_code on demand, large project wings) hold thousands of drawers;
  // building all of them at once freezes the tab. Render the first RENDER_CAP
  // and show a notice to narrow the filter / search for the rest.
  const RENDER_CAP = 500;
  const shown = drawers.slice(0, RENDER_CAP);
  const capNotice = drawers.length > RENDER_CAP
    ? `<div class="empty-list">Showing the first ${RENDER_CAP} of ${drawers.length}. Use search or pick a room to narrow down.</div>`
    : "";

  els.drawerList.innerHTML = capNotice + shown
    .map((drawer) => {
...
async function logout() {
  // BRAIN-PATCH: dashboard auth is Brain's — "Sign out" returns to Brain's UI
  // rather than the (dropped) dashboard login sheet.
  closeSettingsSheet();
  window.location.href = "/";
}
