...
function toolDescribe(name, args) {
  ...
  const descs = {
    ...
    email_accounts: () => 'E-Mail-Konten auflisten',
    email_inbox: () => `Posteingang prüfen${a.account ? ' (' + a.account + ')' : ''}`,
    email_read: () => `E-Mail lesen${a.id ? ' #' + a.id : ''}`,
    email_search: () => `E-Mails suchen: „${a.query || '...'}"`,
    email_send: () => `E-Mail senden an ${a.to || '...'}`,
    email_reply: () => `E-Mail beantworten${a.id ? ' #' + a.id : ''}`,
