...
function openGeneralSettings() {
  ...
  content.innerHTML = `
    ...
    <div class="modal-tabs modal-tabs-vertical" id="general-settings-tabs">
      ...
      <button class="modal-tab" onclick="switchGeneralTab('client-models',this)">Client&nbsp;Models</button>
      <button class="modal-tab" onclick="switchGeneralTab('local-inference',this)">Local&nbsp;Inference</button>
...
async function switchGeneralTab(tab, btn) {
  ...
  if (tab === 'server') {
    try {
      ...
      C.innerHTML = P(`<div style="${G('16px')}">
        ...
        ${SEC('Execution Mode')}
        <div style="display:flex;gap:8px;align-items:center">
          <select class="form-select" id="srv-execution-mode" style="flex:1" onchange="document.getElementById('srv-proxy-tools-section').style.display=this.value==='client'?'block':'none'">
            <option value="server" ${(srv.execution_mode||'server')==='server'?'selected':''}>Server (default)</option>
            <option value="client" ${(srv.execution_mode||'server')==='client'?'selected':''}>Client (air-gapped server)</option>
          </select>
          <button class="btn-secondary" onclick="const m=document.getElementById('srv-execution-mode').value;const tools=[...document.querySelectorAll('.proxy-tool-cb:checked')].map(c=>c.value);API.post('/v1/services/server',{execution_mode:m,client_proxy_tools:tools}).then(()=>{ClientProxy.enabled=(m==='client');if(m==='client')ClientProxy.init();showToast('Execution mode: '+m)}).catch(e=>showToast(e.message||'Failed',true))">Set</button>
        </div>
        <div style="font-size:11px;color:var(--text-400);margin-top:2px">
          <b>Server</b>: LLM calls and web tools execute on the server (requires internet).<br>
          <b>Client</b>: LLM calls and web tools are proxied through the browser. Use when the server has no internet access but clients do.
        </div>
        ${(srv.execution_mode||'server')==='client'?'<div style="font-size:11px;color:var(--warning, #b45309);margin-top:4px;padding:6px 8px;border-radius:6px;background:var(--bg-200)">&#9888; Client execution mode is active. LLM provider calls and web tool requests will be routed through connected browser clients.</div>':''}
        ${(() => {
          const proxyTools = srv.client_proxy_tools || ['web_fetch', 'exa_search'];
          const proxySet = new Set(proxyTools);
          const allTools = srv.available_tools || [];
          const isClient = (srv.execution_mode||'server') === 'client';
          return '<div id="srv-proxy-tools-section" style="margin-top:8px;display:' + (isClient ? 'block' : 'none') + '">'
            + '<div style="font-size:12px;font-weight:500;color:var(--text-200);margin-bottom:6px">Tools proxied through browser:</div>'
            + '<div style="font-size:11px;color:var(--text-400);margin-bottom:6px">Selected tools will execute in the browser instead of on the server. Only select tools that require internet access (the browser must have a JS handler for each).</div>'
            + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px;max-height:200px;overflow-y:auto;border:1px solid var(--border-100);border-radius:8px;padding:6px">'
            + allTools.map(name =>
              '<label style="display:flex;align-items:center;gap:6px;padding:3px 6px;border-radius:4px;cursor:pointer;font-size:11px;color:var(--text-200)">'
              + '<input type="checkbox" class="proxy-tool-cb" value="' + esc(name) + '"' + (proxySet.has(name) ? ' checked' : '') + '>'
              + '<code style="font-size:10px;color:var(--text-300)">' + esc(name) + '</code>'
              + '</label>'
            ).join('')
            + '</div></div>';
        })()}
  ...
  /* ─── CLIENT MODELS (admin manifest of GGUF weights for desktop clients) ─── */
  if (tab === 'client-models') {
    try {
      const mf = await API.get('/v1/client/models/manifest');
      const rows = (mf.models || []);
      const mb = (n) => (n ? (n / 1048576).toFixed(1) + ' MiB' : '—');
      let html = `<div style="${G('12px')}">`;
      html += `<div style="font-size:12px;color:var(--text-400);line-height:1.5">
        GGUF weights the server makes available to Electron desktop clients for local inference. Admin places the file on disk, adds an entry here — clients pull from <code style="font-size:11px">/v1/client/models/&lt;id&gt;/weights</code> and verify by sha256. Family string is the routing key: a session running a chat against a server model with the same family can transparently execute on the client.
      </div>`;
      if (!rows.length) {
        html += `<div style="${ROW};color:var(--text-400);font-size:12px">No client models configured.</div>`;
      } else {
        for (const r of rows) {
          html += `<div style="${ROW};flex-direction:column;align-items:stretch;gap:6px">
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font-weight:600;color:var(--text-100);font-size:13px">${esc(r.id)}</span>
              ${BADGE('family: ' + (r.family || '—'))}
              ${r.auto_download ? BADGE('auto-download', 'var(--accent-brand)') : ''}
              <span style="margin-left:auto;${MONO}">${esc(mb(r.size_bytes))}</span>
              <button class="btn-secondary" style="font-size:11px;padding:2px 8px" onclick="deleteClientModel('${esc(r.id)}')">Delete</button>
            </div>
            <div style="${MONO};word-break:break-all">sha256 ${esc((r.sha256||'').slice(0,16))}&hellip;</div>
          </div>`;
        }
      }
      html += SEC('Add / Update Entry');
      html += `<div style="${G('6px')};padding:12px;border:1px dashed var(--border-100);border-radius:8px">
        <div style="display:grid;grid-template-columns:120px 1fr;gap:6px;align-items:center">
          <label style="font-size:11px;color:var(--text-400)">ID</label>
          <input class="form-input" id="cm-id" placeholder="gemma3-e4b-gguf" style="font-family:var(--font-mono);font-size:12px">
          <label style="font-size:11px;color:var(--text-400)">Family</label>
          <input class="form-input" id="cm-family" placeholder="gemma3-e4b" style="font-family:var(--font-mono);font-size:12px">
          <label style="font-size:11px;color:var(--text-400)">GGUF path</label>
          <input class="form-input" id="cm-path" placeholder="/absolute/path/to/weights.gguf" style="font-family:var(--font-mono);font-size:12px">
          <label style="font-size:11px;color:var(--text-400)">Auto-download</label>
          <label style="display:flex;gap:6px;align-items:center;font-size:12px;color:var(--text-200)">
            <input type="checkbox" id="cm-auto"> Push to clients on connect
          </label>
        </div>
        <div style="display:flex;gap:8px;margin-top:8px">
          <button class="btn-primary" onclick="saveClientModel()">Save</button>
          <span id="cm-status" style="font-size:11px;color:var(--text-400);align-self:center"></span>
        </div>
        <div style="font-size:11px;color:var(--text-400);margin-top:4px">
          sha256 + size are computed on save. Entry with same ID is replaced.
        </div>
      </div>`;
      // Engine manifest (per-platform llama.cpp binaries)
      let engineCfg = {};
      try { engineCfg = await API.get('/v1/client/engines'); } catch {}
      const engines = (engineCfg && engineCfg.engines) || {};
      html += SEC('llama.cpp Engine (per platform)');
      html += `<div style="${G('6px')};padding:12px;border:1px dashed var(--border-100);border-radius:8px">
        <div style="font-size:11px;color:var(--text-400);line-height:1.5;margin-bottom:6px">
          Desktop clients download these binaries lazily on first use. Point URLs at an internal mirror for air-gapped deployments. Edit <code>config.json</code> → <code>client_engines</code> directly (UI for this coming later).
        </div>
        ${['darwin-arm64','darwin-x64','win32-x64','linux-x64'].map(key => {
          const e = engines[key] || {};
          return `<div style="display:grid;grid-template-columns:110px 1fr;gap:6px;align-items:center;padding:3px 0">
            <span style="${MONO}">${esc(key)}</span>
            <span style="${MONO};color:${e.url?'var(--text-200)':'var(--text-400)'}">${esc(e.url || '— not configured —')}</span>
          </div>`;
        }).join('')}
      </div>`;
      html += `</div>`;
      ...
      C.innerHTML = P(`<div style="color:var(--error)">Failed to load client models: ${esc(e.message||String(e))}</div>`);
    }
    return;
  }

  /* ─── LOCAL INFERENCE (per-installation: does THIS browser/desktop run LLMs locally?) ─── */
  if (tab === 'local-inference') {
    const isElectron = !!window.electronAPI;
    const pref = JSON.parse(localStorage.getItem('local-inference') || '{}');
    let manifest = [];
    try {
      const mf = await API.get('/v1/client/models/manifest');
      manifest = Array.isArray(mf.models) ? mf.models : [];
    } catch (e) { /* stay empty */ }
    const enabled = !!(pref.enabled && isElectron);
    const selFamilies = new Set(Array.isArray(pref.families) ? pref.families : []);
    const mb = (n) => (n ? (n / 1048576).toFixed(1) + ' MiB' : '—');
    let html = `<div style="${G('12px')}">`;
    html += `<div style="font-size:12px;color:var(--text-400);line-height:1.5">
      Run LLM inference on this computer instead of the server. Requires the Electron desktop app (Phase 5 bundles llama.cpp; this panel already wires up the toggle + handshake). When enabled for a family that the current chat uses, the server transfers execution here — no queue contention with other users of the central GPU, and it keeps LLM calls local on air-gapped deployments.
    </div>`;
    if (!isElectron) {
      html += `<div style="${ROW};background:var(--bg-200);color:var(--text-300);font-size:12px">
        &#9432; Local inference is only available in the desktop app. In a regular browser this tab is read-only.
      </div>`;
    }
    html += `<div style="${ROW}">
      <label style="display:flex;gap:10px;align-items:center;flex:1;cursor:${isElectron?'pointer':'not-allowed'}">
        <input type="checkbox" id="li-enabled" ${enabled?'checked':''} ${isElectron?'':'disabled'}>
        <span style="font-weight:500;color:var(--text-100);font-size:13px">Enable local inference on this installation</span>
      </label>
    </div>`;
    html += SEC('Available Models');
    if (!manifest.length) {
      html += `<div style="${ROW};color:var(--text-400);font-size:12px">No client-eligible models published by the server. Ask an admin to add entries in the Client Models tab.</div>`;
    } else {
      for (const m of manifest) {
        const chosen = selFamilies.has(m.family);
        html += `<div style="${ROW};flex-direction:column;align-items:stretch;gap:6px">
          <div style="display:flex;align-items:center;gap:8px">
            <input type="checkbox" class="li-family-cb" data-family="${esc(m.family)}" ${chosen?'checked':''} ${isElectron?'':'disabled'}>
            <span style="font-weight:600;color:var(--text-100);font-size:13px">${esc(m.id)}</span>
            ${BADGE('family: ' + (m.family||'—'))}
            ${m.auto_download ? BADGE('auto-download', 'var(--accent-brand)') : ''}
            <span style="margin-left:auto;${MONO}">${esc(mb(m.size_bytes))}</span>
          </div>
          <div style="${MONO};word-break:break-all">sha256 ${esc((m.sha256||'').slice(0,16))}&hellip; &middot; ${esc(m.download_path||'')}</div>
        </div>`;
      }
    }
    html += `<div style="display:flex;gap:8px;margin-top:12px">
      <button class="btn-primary" onclick="saveLocalInferenceSettings()">Save</button>
      <button class="btn-secondary" onclick="prefetchLocalInference()" ${isElectron?'':'disabled'}>Download now</button>
      <span id="li-status" style="font-size:11px;color:var(--text-400);align-self:center"></span>
    </div>`;
    html += `<div id="li-progress" style="display:none;margin-top:8px;padding:10px;border:1px solid var(--border-100);border-radius:8px">
      <div style="font-size:12px;color:var(--text-300);margin-bottom:4px" id="li-progress-label">Downloading…</div>
      <div style="height:4px;background:var(--bg-200);border-radius:2px;overflow:hidden">
        <div id="li-progress-bar" style="height:100%;width:0%;background:var(--accent-brand);transition:width 0.2s"></div>
      </div>
    </div>`;
    html += `</div>`;
    C.innerHTML = P(html);
    // Wire up progress callback for this tab view (replaced each time the
    // tab is opened; the module keeps listeners live across tab switches).
    window.onLocalInferenceProgress = (p) => {
      const bar = document.getElementById('li-progress-bar');
      const panel = document.getElementById('li-progress');
      const label = document.getElementById('li-progress-label');
      if (!panel) return;
      panel.style.display = '';
      const pct = Math.min(100, Math.max(0, (p.fraction || 0) * 100)).toFixed(1);
      if (bar) bar.style.width = pct + '%';
      if (label) {
        const what = p.kind === 'engine' ? 'llama.cpp engine' : ('weights ' + (p.model_id || ''));
        label.textContent = `Downloading ${what} — ${pct}%`;
      }
      if (p.fraction >= 1) {
...
async function prefetchLocalInference() {
  const statusEl = document.getElementById('li-status');
  if (!window.electronAPI?.localInference) {
    if (statusEl) statusEl.textContent = 'Desktop app required';
    return;
  }
  const selected = [...document.querySelectorAll('.li-family-cb:checked')]
    .map(cb => cb.dataset.family).filter(Boolean);
  const serverUrl = (await window.electronAPI.getServerUrl?.()) || location.origin;
  const authToken = localStorage.getItem('brain-auth-token') || '';
  try {
    if (statusEl) statusEl.textContent = 'Downloading engine…';
    const eRes = await window.electronAPI.localInference.ensureEngine({ serverUrl, authToken });
    if (!eRes.ok) throw new Error(eRes.error);
    for (const family of selected) {
      const entry = (LocalInference.manifest || []).find(m => m.family === family);
      if (!entry) continue;
      if (statusEl) statusEl.textContent = `Downloading ${entry.id}…`;
      const r = await window.electronAPI.localInference.ensureModel(entry, serverUrl, authToken);
      if (!r.ok) throw new Error(`${entry.id}: ${r.error}`);
      LocalInference.modelReadySha.add(entry.sha256);
    }
    LocalInference.engineReady = true;
    if (statusEl) statusEl.textContent = 'All assets cached';
    showToast('Local inference ready');
  } catch (e) {
    if (statusEl) statusEl.textContent = 'Failed: ' + (e.message || e);
    showToast('Prefetch failed: ' + (e.message || e), true);
  }
}

async function saveLocalInferenceSettings() {
  const enabled = !!document.getElementById('li-enabled')?.checked;
  const families = [...document.querySelectorAll('.li-family-cb:checked')]
    .map(cb => cb.dataset.family).filter(Boolean);
  const statusEl = document.getElementById('li-status');
  localStorage.setItem('local-inference', JSON.stringify({ enabled, families }));
  if (window.LocalInference) {
    LocalInference.enabled = enabled && !!window.electronAPI;
    LocalInference.families = families;
  }
  // Re-declare capabilities for every open chat so the server picks up the
  // new setting on the next turn without requiring a session re-open.
  try {
    for (const chat of Object.values(state.agentChats || {})) {
      if (chat?.sessionId && window.LocalInference) {
        await LocalInference.declareCapabilities(chat.sessionId);
      }
    }
    if (statusEl) statusEl.textContent = 'Saved · capabilities re-declared';
    showToast('Local inference settings saved');
  } catch (e) {
    if (statusEl) statusEl.textContent = 'Saved but handshake failed: ' + (e.message || e);
  }
}

async function saveClientModel() {
  const id = document.getElementById('cm-id')?.value?.trim();
  const family = document.getElementById('cm-family')?.value?.trim();
  const gguf_path = document.getElementById('cm-path')?.value?.trim();
  const auto_download = !!document.getElementById('cm-auto')?.checked;
  const statusEl = document.getElementById('cm-status');
  if (!id || !family || !gguf_path) {
    if (statusEl) statusEl.textContent = 'id, family, and path are required';
    return;
  }
  try {
    if (statusEl) statusEl.textContent = 'Hashing on server…';
    await API.post('/v1/client/models', {
      action: 'add',
      model: { id, family, gguf_path, auto_download },
    });
    showToast('Client model saved');
    switchGeneralTab('client-models', document.querySelector(".modal-tab[onclick*=\"'client-models'\"]"));
  } catch (e) {
    if (statusEl) statusEl.textContent = 'Failed: ' + (e.message || e);
...
async function deleteClientModel(id) {
  if (!confirm(`Delete client model "${id}"?\n\nThis only removes the manifest entry. The GGUF file on disk is untouched.`)) return;
  try {
    await API.post('/v1/client/models', { action: 'delete', id });
    showToast('Client model removed');
    switchGeneralTab('client-models', document.querySelector(".modal-tab[onclick*=\"'client-models'\"]"));
  } catch (e) {
