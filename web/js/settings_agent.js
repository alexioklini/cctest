...
async function searchCCMarketplace(query) {
  ...
  results.innerHTML = `<div style="padding:20px;text-align:center">
    <div style="display:inline-block;width:18px;height:18px;border:2px solid #e8e7e0;border-top-color:#3b82f6;border-radius:50%;animation:spin 0.6s linear infinite"></div>
        ...
        <div>
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
            <div style="display:flex;align-items:center;gap:8px">
              ...
              <span style="font-size:10px;padding:2px 6px;border-radius:4px;background:rgba(59,130,246,0.12);color:#3b82f6">${agentSkills.length} installiert</span>
            </div>
          </div>`;
          ...
          html += `
            <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border:1px solid rgba(31,30,29,0.08);border-radius:8px;opacity:${s.agent_enabled ? '1' : '0.6'};transition:background 0.15s,opacity 0.2s;overflow:hidden" onmouseover="this.style.background='rgba(0,0,0,0.02)'" onmouseout="this.style.background='transparent'">
              <label style="position:relative;width:36px;height:20px;flex-shrink:0;cursor:pointer">
                ...
                <span style="position:absolute;inset:0;border-radius:10px;background:${s.agent_enabled ? '#3b82f6' : '#e8e7e0'};transition:background .2s;cursor:pointer"></span>
      ...
      html += `
        <div style="border-top:1px solid rgba(31,30,29,0.08);padding-top:16px">
          ...
          <div style="position:relative;margin-bottom:10px">
            <input id="cc-marketplace-search" style="width:100%;box-sizing:border-box;background:#f5f4ed;border:1px solid rgba(31,30,29,0.08);border-radius:8px;padding:9px 80px 9px 12px;font-size:13px;color:#141413;outline:none;transition:border-color 0.15s" placeholder="Claude Code Plugins durchsuchen…" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(31,30,29,0.08)'" onkeydown="if(event.key==='Enter'){event.preventDefault();searchCCMarketplace(this.value)}" oninput="clearTimeout(window._ccMktDebounce);window._ccMktDebounce=setTimeout(()=>searchCCMarketplace(this.value),400)">
            <button id="cc-marketplace-search-btn" style="position:absolute;right:4px;top:50%;transform:translateY(-50%);font-size:11px;padding:5px 14px;border-radius:6px;background:#3b82f6;color:#fff;border:none;cursor:pointer;font-weight:500;transition:background 0.15s" onmouseover="this.style.background='#2563eb'" onmouseout="this.style.background='#3b82f6'" onclick="searchCCMarketplace(document.getElementById('cc-marketplace-search').value)">Suchen</button>
          </div>
    ...
    container.innerHTML = `
      <div style="padding:16px;display:grid;gap:14px">
        <div style="display:flex;align-items:center;justify-content:space-between">
          ...
          <button style="font-size:11px;padding:5px 14px;border-radius:6px;background:#3b82f6;color:#fff;border:none;cursor:pointer;font-weight:500;transition:background 0.15s" onmouseover="this.style.background='#2563eb'" onmouseout="this.style.background='#3b82f6'" onclick="_showAgentMcpAdd('${esc(agentId)}')">+ Manuell hinzufügen</button>
        </div>
        ...
        <div style="border-top:1px solid rgba(31,30,29,0.08);padding-top:16px">
          ...
          <div style="position:relative;margin-bottom:10px">
            <input id="mcp-registry-search" style="width:100%;box-sizing:border-box;background:#f5f4ed;border:1px solid rgba(31,30,29,0.08);border-radius:8px;padding:9px 80px 9px 12px;font-size:13px;color:#141413;outline:none;transition:border-color 0.15s" placeholder="Server suchen (z. B. github, slack, postgres)…" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(31,30,29,0.08)'" onkeydown="if(event.key==='Enter'){event.preventDefault();_searchMcpRegistry('${esc(agentId)}')}" oninput="clearTimeout(window._mcpSearchDebounce);window._mcpSearchDebounce=setTimeout(()=>_searchMcpRegistry('${esc(agentId)}'),400)">
            <button id="mcp-registry-search-btn" style="position:absolute;right:4px;top:50%;transform:translateY(-50%);font-size:11px;padding:5px 14px;border-radius:6px;background:#3b82f6;color:#fff;border:none;cursor:pointer;font-weight:500;transition:background 0.15s" onmouseover="this.style.background='#2563eb'" onmouseout="this.style.background='#3b82f6'" onclick="_searchMcpRegistry('${esc(agentId)}')">Suchen</button>
          </div>
...
// Shared inline styles (accent = #3b82f6 brand blue)
...
const _mcpBtn = 'font-size:11px;padding:5px 14px;border-radius:6px;background:#3b82f6;color:#fff;border:none;cursor:pointer;font-weight:500;white-space:nowrap';
  ...
  try {
    ...
    container.innerHTML = `<div style="font-size:11px;color:#a1a09a;margin-bottom:2px">${servers.length} Server gefunden</div>` + servers.map((s, idx) => {
      ...
      const installBtn = alreadyInstalled
        ...
        : `<button style="${_mcpBtn};padding:4px 12px;font-size:11px" onmouseover="this.style.background='#2563eb'" onmouseout="this.style.background='#3b82f6'" onclick="_installFromRegistry('${esc(agentId)}',${idx},this)">Hinzufügen</button>`;
      return `
